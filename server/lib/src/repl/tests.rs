...
async fn test_repl_increment_basic_entry_tombstone(server_a: &QueryServer, server_b: &QueryServer) {
    ...
    assert!(e1.attribute_equality("class", &AcpClass::Tombstone.into()));
...
async fn test_repl_increment_basic_bidirectional_lifecycle(
    ...
    // They are consistent again.
    assert!(e1 == e2);
    assert!(e1.attribute_equality("class", &AcpClass::Recycled.into()));
    ...
    assert!(e2.attribute_equality("class", &AcpClass::Tombstone.into()));
...
async fn test_repl_increment_schema_conflict(server_a: &QueryServer, server_b: &QueryServer) {
    ...
    let modlist = ModifyList::new_list(vec![
        Modify::Removed("class".into(), AcpClass::Person.into()),
        Modify::Present("class".into(), AcpClass::Group.into()),
    ...
    assert!(e1.attribute_equality("class", &AcpClass::Conflict.into()));
    ...
    assert!(e2.attribute_equality("class", &AcpClass::Conflict.into()));
