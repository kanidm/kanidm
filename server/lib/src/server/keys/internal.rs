...
impl KeyObjectInternalJweA128GCM {
    ...
    fn to_key_iter(&self) -> impl Iterator<Item = (KeyId, KeyInternalData)> + '_ {
        self.all.iter().map(|(key_id, internal_jwe)| {
            ...
            let (status, der) = match &internal_jwe.status {
                ...
                InternalJweA128GCMStatus::Revoked => (KeyStatus::Revoked, Zeroizing::new(Vec::with_capacity(0))),
            };
    ...
    fn load(
        ...
        let status = match status {
            KeyStatus::Valid => {
                let key = aes128::key_from_slice(der)
                .ok_or_else(|| {
...
impl KeyObjectInternalJwtHs256 {
    ...
    fn import(
        ...
        for private_bytes in import_keys {
            ...
            self.all.insert(
                ...
                InternalJwtHs256 {
                    valid_from,
                    status: InternalJwtHs256Status::Valid {
                        verifier,
                    },
                    status_cid: cid.clone(),
                },
    ...
    fn load(
        ...
        let status = match status {
            ...
            KeyStatus::Retained => {
                ...
                InternalJwtHs256Status::Retained {
                    verifier,
                }
            }
    ...
    fn public_jwks(&self) -> JwkKeySet {
        JwkKeySet { keys: Vec::with_capacity(0) }
    }
