...
pub(crate) fn qs_test(_args: TokenStream, item: TokenStream) -> TokenStream {
...
pub(crate) fn qs_test_no_init(_args: TokenStream, item: TokenStream) -> TokenStream {
    let input: syn::ItemFn = match syn::parse(item.clone()) {
        Ok(it) => it,
        Err(e) => return token_stream_with_error(item, e),
    };
    ...
    let result = quote! {
        ...
        fn #test_driver() {
            let body = async {
                let test_server = crate::testkit::setup_test_no_init().await;
