    ...
    pub fn backup(&self,  audit: &mut AuditScope, dstPath: &str) -> Result<(), OperationError> {
        ...
        match result {
            ...
            Err(e) => {
                ...
                // since we are reporting an error of type SQLiteError there is no need to embed the
                // RusqliteError here, we can just point the admin to the audit_log that will have
                // the error within it.
