...
async fn preflight_person(
    ...
    if client.person_exists(&person.username).await? {
        ...
        client.person_create(&person.username, &person.display_name).await?;
    }
    ...
    match &person.credential {
        Credential::Password {
            ...
            client.person_set_pirmary_password_only(&person.username, &plain).await?;
        }
    }
